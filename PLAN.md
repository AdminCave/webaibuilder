...
- [ ] UI-Icons: aktuell lucide-react hinter der `<Icon>`-Registry (`renderer/src/components/icons.ts`) — bei Bedarf später durch eigene AdminCave-DS-Icons ersetzen (nur die Registry tauschen)
- [ ] „Benannte Versionen": `nameVersion` (annotated Tags) ist in `packages/versioning` fertig, hat aber noch keinen IPC-Kanal/keine UI (Timeline-Aktion „Version benennen")
...
- [x] **Politur-Runde „Chat freischalten · Settings-Redesign · Icons" (2026-07-15)**:
  - *Chat nutzbar (Kernproblem „erkennt Claude, aber nichts geht"):* env-Key (`ANTHROPIC_API_KEY` & Co.) zählt jetzt auch für die Freischaltung, nicht nur für die Erkennung (`PROVIDER_ENV_KEYS`, `apiKeySource`; Keychain gewinnt). Eine geteilte Readiness-Quelle `chatBlockReason` (shared/backends). Chat-Empty-State mit geführtem Einrichtungs-Pfad (`recommendChatSetup`): „Claude (Abo) gefunden — jetzt einrichten" → Hinweis → Bestätigen → Aktivieren in einem Fluss (Ack bleibt explizit, PLAN §3). Echte Login-Probe (`claude auth status` JSON / `codex login status`, fail-safe) → „eingeloggt als …" statt ewig „gefunden". Fehlerursachen (401, Modell) bis in die UI (aufklappbare Details + `errorHints`); `settings.get`-Fehler als Banner mit Retry. Watchdog in `cliEngine` (Default 120 s, pausiert bei offener Permission) gegen still hängende CLIs.
  - *Settings-Redesign:* Modal mit Seitennavigation (KI & Backends · Darstellung · Hilfe & Logs, `shared/settingsNav.ts`), EIN Aktivierungsweg für alle sechs Backends (Karten mit Inline-Formular; BackendPicker + altes Formular-Doppel entfernt), Deep-Links (Chat → byok-Karte), Strg/Cmd+, + Escape.
  - *Icons:* `lucide-react` hinter eigener `<Icon>`-Komponente + Registry (`icons.ts` = einzige Austausch-Schicht für spätere DS-Icons); Titlebar, Settings-Nav, Backend-Status (check/dot/alert), Chat, Preview, Timeline, StatusBar, Deploy.
  - *Robustheit:* App-Start crash-sicher (try/catch + Fehlerdialog statt fensterlosem Prozess); `pnpm dev`/`test` togglen die better-sqlite3-ABI automatisch (Marker, `rebuild-native.mjs --if-needed`); React Error Boundary app-weit + pro Panel; zod-Validierung der IPC-Argumente (`ipcSchemas.ts`); CI führt jetzt `pnpm test` aus.
  - *Flow-Fixes:* Restore-/Vorlagen-/Projektlisten-Fehler sichtbar (statt still), Preview-„Erneut versuchen" im Fehlerfall, proaktive Drift-Prüfung (deploy.drift-Kanal endlich angeschlossen), Deploy-`outcome` gerendert, Kosten-Chip nur bei echten Kosten, Projekt umbenennen/entfernen im StartScreen.
  - Tests: 333 grün (Desktop 239, agents 55, preview 8, deploy 16, versioning 15); Typecheck/Lint/Build grün. GUI-Smoke (Onboarding→Chat→Deploy in beiden Themes) steht als manueller Check aus — diese Runde lief in einer TTY-Sitzung ohne Display.
